...
def orderStatusUpdate():
    for order_obj in order2.objects.all():
        if order_obj.expireDateTime < timezone.now():
            order_obj.order_status = order2.expired
            order_obj.save()
...
def search(request):
    ...
    results = order2.objects.filter(order_status=order2.uncompleted, free_lancer=None,campus=campus,kind =kind).values(
        *["orderid", "createTime", "money", "received_pos","expireDateTime",
          "goods_introduction", "goods_category", "goods_img","campus"])
    print(len(results))
...
def getOrder(request):  # 获得某个订单的具体信息
    ...
    #campus = request.GET.get("Ocampus","")#新加
    user_exists = True
    try:
    ...
    ret_order = ""
    try:
        ret_order = order2.objects.get(orderid=orderid)
    ...
    print(ret_values)
    return JsonResponse({"order2": ret_values}, safe=False)
...
def sendOrder(request):
    if request.method == 'POST':
        ...
        if not cur_user or not goods_introduction or not goods_weight or not money or not campus or not received_pos \
                or not kind or not pos or not expireTime or not goods_category:
            ...
            newOrder.order_status = order2.uncompleted
...
def receiveOrder(request):
    ...
    if cur_user.received_order_count <= 0:
        ...
        cur_order.order_status = order2.completing
...
def cancelOrder(request):
    ...
    if cur_order.order_owner == cur_user:
        if cur_order.order_status == order2.canceled:
            return JsonResponse({"msg": "订单早已取消"}, status=404)
        elif cur_order.order_status != order2.uncompleted|order2.completing:
            return JsonResponse({"msg": "订单已完成或过期不能取消"}, status=404)
        cur_order.order_status = order2.canceled
        cur_user.sended_order_count += 1
...
def calRate(cur_user: user):
    owner_orders = order2.objects.filter(order_owner=cur_user)
    lancer_orders = order2.objects.filter(free_lancer=cur_user)
    ...
    for orderObj in owner_orders:
        try:
            commentObj = comment.objects.get(order2=orderObj)
...
def orderComplete(request):
    ...
    cur_order = get_object_or_404(order2, orderid=orderid)
    if cur_order.order_owner == cur_user:
        if cur_order.order_status == order2.completed:
            return JsonResponse({"msg": "订单早已完成"}, status=404)
        elif cur_order.order_status != order2.completing:
            return JsonResponse({"msg": "订单已取消或过期不能完成"}, status=404)
        cur_order.order_status = order2.uncommented
        cur_order.save()
        cur_user.sended_order_count += 1
        lancer_user = cur_order.free_lancer
        lancer_user.received_order_count += 1
